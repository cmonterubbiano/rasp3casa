<?php 
	...
	if ($result->num_rows <1) {
		$query = "SELECT * FROM sensori WHERE `sensori`.`id` !='1'";
		$result = executeQuery($query);
		//echo "DDDDDDDDDDDDDDDDDDDDDDD ". $result->num_rows;
	}
...
<?php 	
	...
	if(isset($_POST['nome_job'])){ 
		...
		$result = executeQuery($query);
		if($result){
			echo "<h1>JOB inserito correttamente.</h1>";
			echo "<h2>Inoltro a AUTOMAZIONE CONSUMI....</h2>";
			header('Refresh: 2; ./smart_control.php'); 
		} else {
...
<!--<form method="post" onsubmit="return checkFormFields();"> -->
        ...
        <tbody>
			...
			<tr>
				...
				<button onclick="myFunction()" class="new_job_submit">Leggi Sensore</button><br>
...
<form method="post" onsubmit="return checkFormFields();">		
			...
			<tr>
				...
				<button onclick="myFunction()" class="new_job_submit">ANNULLA SENSORE</button><br>
	...
	*/
    ...
    $.post("recuperaDatiSensore.php",
		...
		function(data, status){
			...
			if (data)
			{
				$('#new_job_nome_job').val( data.descrizione);
